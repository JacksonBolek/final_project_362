...
xgb<-train(data=train_dummy, as.factor(deposit)~., method="xgbTree", trControl=trainControl(method="cv", number=5, verbose=T, classProbs=TRUE, summaryFunction=mnLogLoss), tuneGrid=expand.grid(eta=c(.005), max_depth=c(10), gamma=c(.2), colsample_bytree=1, subsample=.9, nrounds=2000, min_child_weight=2), metric="logLoss")
...
```{r}
ranger<-train(data=train_dummy, as.factor(deposit)~., method="ranger", trControl=trainControl(method="cv", number=5, verbose=T, classProbs=T, summaryFunction=mnLogLoss), tuneGrid=expand.grid(mtry=7:8, min.node.size=c(1,2), splitrule="gini"), metric="logLoss")
...
compete.final<-data.frame(id=1:nrow(compete), prob=pred_compete[,2]) %>% write_csv(file="proudpigeons_6")
